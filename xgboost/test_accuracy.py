"""
Test and Accuracy Evaluation Script for XGBoost Baseline.

Evaluates:
1. Action concordance accuracy against Gold-Standard ADA Clinical Guidelines.
2. Concordance accuracy against historical clinician behavior.
...
def evaluate_xgboost_accuracy(
    ...
    if model_path is None:
        model_path = Path(__file__).resolve().parent / "xgboost_model.json"

    if not model_path.exists():
        raise FileNotFoundError(f"Model file not found at {model_path}. Run train.py first.")

    agent = XGBoostAgent(num_class=config.NUM_ACTIONS)
    ...
    for split_name, split_df in [("Train", train_df), ("Test", test_df)]:
        ...
        for _, group in grouped:
            ...
            for t in range(T):
                ...
                a_pred = agent.select_action(s_vec, deterministic=True)
    ...
    print("\n" + "=" * 65)
    print("      XGBOOST BASELINE PERFORMANCE EVALUATION (TEST SET)")
    print("=" * 65)
    ...
    if save_plot:
        plot_path = Path(__file__).resolve().parent / "xgboost_confusion_matrix.png"
